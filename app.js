...
// ✨ FIX #1: The Fatal HTML Escape bug (100% Syntax Safe) ✨
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log("Ultimate Hydro Pro v4.0 Booting...");
    try {
        await idb.init(); 
...
window.renderHome = () => {
    ...
    // ✨ FIX #2: The Smart Time of Day Greeting ✨
    ...
    // ✨ FIX #3: Strict string matching to prevent missing cards on Route/Home ✨
...
window.triggerAI = (context, id = null) => {
    ...
    if(!key) { openAITeaserModal(); } 
        ...
        if(context === 'voice') showToast("Listening... (Awaiting API config)", "normal");
        if(context === 'receipt') showToast("Scanning receipt... (Awaiting API config)", "normal");
        if(context === 'reply') showToast("Drafting reply... (Awaiting API config)", "normal");
    }
};
...
window.saveCustomer = () => {
    ...
    // UX Upgrade: Automatically jump to the day of the customer you just added!
    curWeek = parseInt(newDetails.week);
    workingDay = newDetails.day;
    localStorage.setItem('HP_curWeek', curWeek);
    localStorage.setItem('HP_workingDay', workingDay);
    
    document.querySelectorAll('.segment').forEach(b => { if(b.id && b.id.startsWith('wk-btn-')) b.classList.remove('active'); });
    const activeWkBtn = document.getElementById(`wk-btn-${curWeek}`); if(activeWkBtn) activeWkBtn.classList.add('active');
    
    document.querySelectorAll('.WEE-day-btn').forEach(b => b.classList.remove('active'));
    const activeDayBtn = document.getElementById(`day-${workingDay}`); if(activeDayBtn) activeDayBtn.classList.add('active');
...
// ✨ FIX #3: Strict Filtering for missing Route Cards ✨
window.renderWeek = () => { 
    const list = document.getElementById('WEE-list-container'); if(!list) return; list.innerHTML = '';
    
    // Strict string matching solves the bug where jobs don't appear
