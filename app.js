...
document.addEventListener('DOMContentLoaded', async () => {
    console.log("Ultimate Hydro Pro v3.2 Booting...");
    try {
        await idb.init(); 
...
window.renderHome = () => {
    ...
    let todaysJobs = db.customers.filter(c => {
        let dbWeek = String(c.week || "1").trim();
        let dbDay = String(c.day || "Mon").trim();
        let currentWk = String(curWeek).trim();
        let currentDay = String(workingDay).trim();
        return dbWeek === currentWk && dbDay === currentDay && !c.skipped;
    });
...
// ✨ FIXED ROUTE RENDERING (Bulletproof String Comparison to find missing customers!) ✨
window.renderWeek = () => { 
    const list = document.getElementById('WEE-list-container'); if(!list) return; list.innerHTML = '';
    
    let customersToday = db.customers.filter(c => {
        let dbWeek = String(c.week || "1").trim();
        let dbDay = String(c.day || "Mon").trim();
        let currentWk = String(curWeek).trim();
        let currentDay = String(workingDay).trim();
        return dbWeek === currentWk && dbDay === currentDay;
    }).sort((a, b) => { 
